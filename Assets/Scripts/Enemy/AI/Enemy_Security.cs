...
public class Enemy_Security : EnemyScript {
	...
	{
		levelValue.Add (0, State.Stun);
		levelValue.Add (1, State.Escape);
		levelValue.Add (2, State.Attack);
		levelValue.Add (3, State.Alert);
		levelValue.Add (4, State.Detection);
		levelValue.Add (5, State.Suspicious);
		levelValue.Add (6, State.Patrol);
		levelValue.Add (7, State.Idle);
	}

	public int GetStateLayerKey(State s)
	{
		for(int i = 0; i < levelValue.Count; i++)
		{
			if(levelValue[i] == s)
			{
				return i;
