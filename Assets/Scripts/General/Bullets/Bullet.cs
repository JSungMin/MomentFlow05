...
public class Bullet : MonoBehaviour
{
    ...
    {
        InitBoxColliderInfo();
        gHitInfo = new RaycastHit[rayDensity][];

        for (int i = 0; i < rayDensity; i++)
        {
            if (dir.x >= 0)
            {
                gHitInfo[i] = Physics.RaycastAll(Vector3.right * bc.bounds.max.x + Vector3.up * bc.bounds.max.y * (i / (rayDensity - 1)),
                    Vector3.right * dir.x, Mathf.Abs(dir.x * speed * Time.deltaTime),
                    collisionMask);
            }
            else
            {
                gHitInfo[i] = Physics.RaycastAll(Vector3.right * bc.bounds.min.x + Vector3.up * bc.bounds.max.y * (i / (rayDensity - 1)),
                    Vector3.right * dir.x, Mathf.Abs(dir.x * speed * Time.deltaTime),
                    collisionMask);
            }
    ...
    protected void RaycastVertical()
    {
        ...
        for (int i = 0; i < rayDensity; i++)
        {
            if (dir.y >= 0)
            {
                gHitInfo[i] = Physics.RaycastAll(Vector3.up * bc.bounds.max.y + Vector3.right * bc.bounds.max.x * (i / (rayDensity - 1)),
                    Vector3.up * dir.y, Mathf.Abs(dir.y * speed * Time.deltaTime),
                    collisionMask);
            }
            else
            {
                gHitInfo[i] = Physics.RaycastAll(Vector3.up * bc.bounds.min.y + Vector3.right * bc.bounds.max.x * (i / (rayDensity - 1)),
                    Vector3.up * dir.y, Mathf.Abs(dir.y * speed * Time.deltaTime),
                    collisionMask);
            }
            for (int j = 0; j < gHitInfo[i].Length; j++)
